...
public class CookieHelper {
	...
	private final static Logger logger                  = LoggerFactory.getLogger("ssoLogin");
	...
	public static void clearAllCookie(HttpServletRequest request, HttpServletResponse response, String domain,
		...
		logger.info("[Browser-clearAllCookie] clearAllCookie in  domain " + domain);
	}
	...
	private static boolean clearCookie(HttpServletResponse response, String cookieName, String domain, String path) {
		...
		try {
			...
			cookie.setPath(path);
			response.addCookie(cookie);
			logger.info("[Browser-clearCookie] clear cookie " + cookieName);
			result = true;
		} catch (Exception e) {
			logger.error("[Browser-clearCookie] clear cookie " + cookieName + " is exception!\n", e);
		}
		return result;
	}
